  ...
  return (
    <div className="fixed bottom-8 right-8 z-50">
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="max-w-md bg-white/95 backdrop-blur-md border-slate-200">
          <DialogHeader>
          ...
          <div className="space-y-6 mt-4">
            {sections.map((section, idx) => (
              <div key={idx} className="bg-slate-50 p-4 rounded-xl border border-slate-100 group hover:border-primary/20 transition-all">
          ...
          <div className="mt-6 flex justify-center">
             <Button variant="outline" asChild className="rounded-full w-full">
               <a href="/guide" onClick={() => setIsOpen(false)}>View Full Guide</a>
             </Button>
          </div>
        </DialogContent>
      </Dialog>
